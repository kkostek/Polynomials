...
/**
 * Wypisuje na standardowe wyjÅ›cie informacjÄ™ o argumentach programu
 ...
 * @param op funkcja przyjmujÄ…ca dwa wielomiany jako argument i zwracajÄ…ca wielomian
 * (DomyÅ›lnie jedna z PolyAdd , PolySub, PolyMul)
 */
...
/**
 * Funkcja testujÄ…ca dodawanie, odejmowanie i mnoÅ¼enie liczb
 * za pomocÄ… wielomianÃ³w
 */
bool SimpleArithmeticTest()
{
...
/**
 * Funkcja budujÄ…ca coraz dÅ‚uÅ¼sze wielomiany
 */
bool LongPolynomialTest()
{
    bool res = true;
...
/**
 * Test czy funkcje PolyAddMonos i MonoFromPoly przejmujÄ… na wÅ‚asnoÅ›Ä‡
 * jednomiany i monomiany.
 * Uruchomione pod valgrindem nie powinno dawaÄ‡ wyciekÃ³w pamiÄ™ci
 */
void MemoryThiefTest()
{
...
/**
 * Sprawdza czy stopieÅ„ wielomianu poprawnie siÄ™ zmienia
...
/**
 * Prosty test ewaluacji (PolyAt) wielomianÃ³w.
 * Testuje czy dobrze sÄ… dobrane typy danych.
 * ZaÅ‚ada, Å¼e poly_coef_r jest typu long
 */
bool SimpleAtTest2()
{
    Poly p_one = PolyFromCoeff(1);
 ...
 *
 * NastÄ™pnie testowane jest PolyDeg i PolyDegBy
 * czy dziaÅ‚aja na prostym przypadku
 */
bool AtTest()
{
...
/**
 * Tworzenie wielomianu jednej zmiennej z listy wspÃ³Å‚czynnikÃ³w i wykÅ‚adnikÃ³w
 * @param count rozmiar tablicy
 * @param val tablica wspÃ³Å‚czynnikÃ³w
 * @param exp tablica wykÅ‚adnikÃ³w
 */
...
/**
 * Tworzenie wielomianu wielu zmiennych z listy wielomianÃ³w i wykÅ‚adnikÃ³w
 * @param count rozmiar tablicy
 * @param val tablica wspÃ³Å‚czynnikÃ³w
 * @param exp tablica wykÅ‚adnikÃ³w
 */
...
/**
 * Buduje wielomian o okreÅ›lonej glebokoÅ›ci. Pobiera dÅ‚ugoÅ›Ä‡ kolejnych
 * wielomianÃ³w z tablicy exp_arr i wykÅ‚adniki z tablicy exp_arr1
 * wspÃ³Å‚czynniki sÄ… z tablicy coef_arr1.
 * @param depth gÅ‚Ä™bokoÅ›Ä‡  (iloÅ›Ä‡ zmiennych) wielomianu
 * @param exp_shift wkaÅºnik do zmiennej przechowywujÄ…cej pozycje w
 * tablicach exp_arr i exp_arr2 (z const_arr.h)
 * @param coef_shift wkaÅºnik do zmiennej przechowywujÄ…cej pozycje w
...
/**
 * Buduje wielomian o okreÅ›lonej glebokoÅ›ci. Pobiera dÅ‚ugoÅ›Ä‡ kolejnych
 * wielomianÃ³w z tablicy exp_arr i wykÅ‚adniki z tablicy exp_arr (argument)
 * wspÃ³Å‚czynniki sÄ… z tablicy coef_arr(argument).
 * @param depth gÅ‚Ä™bokoÅ›Ä‡ (iloÅ›Ä‡ zmiennych) wielomianu
 * @param exp_shift wkaÅºnik do zmiennej przechowywujÄ…cej pozycje w
 * tablicy exp_arr
 * @param coef_shift coef_shift wkaÅºnik do zmiennej przechowywujÄ…cej pozycje w
 * tablicy coef_arr
 * @param coef_arr tablica wspÃ³Å‚czynnikÃ³w
 * @param exp_arr tablica wykÅ‚adnikÃ³w i dÅ‚ugoÅ›ci
 * @return
 */
...
/**
 * Sprawdza czy PolyDegBy i PolyDeg przeglÄ…dajÄ… wszystkie potrzebne
 * elementy struktury
 */
bool DegByTest()
{
...
/**
 * Sprawdza mnoÅ¼enie na wzorach skrÃ³conego mnoÅ¼enia
 */
bool MulTest()
{
    {
 ...
 * gdzie kaÅ¼da zmienna wystÄ™puje w dokÅ‚adnie jednym jednomianie
 * @param coef_arr
 * @param exp_arr
 * @param rec
 */
...
/**
 * Testuje czy dodawanie dÅ‚ugich wielomianÃ³w jednej zmiennej dziaÅ‚a poprawnie
 * Testuje dziaÅ‚anie PolyNeg
 */
bool AddTest1()
{
...
/**
 * Testuje czy dodawanie dÅ‚ugich wielomianÃ³w wielu zmiennych dziaÅ‚a poprawnie
 * Testuje dziaÅ‚anie PolyNeg
 */
bool AddTest2()
{
...
/**
 * Testuje czy odejmowanie dÅ‚ugich wielomianÃ³w jednej zmiennej dziaÅ‚a poprawnie
 */
bool SubTest1()
{
    const poly_exp_t step = 10;
...
/**
 * Testuje czy odejmowanie dÅ‚ugich wielomianÃ³w wielu zmiennych dziaÅ‚a poprawnie
 */
bool SubTest2()
{
    bool good = true;
...
/**
 * Funkcja pomocnicza do tworzenie wynikÃ³w mnoÅ¼enia wielomianow 
 * @param size1 dÅ‚ugoÅ›Ä‡ danych w arr_1
 * @param arr_1 tablica wspolczynnikow pierwszego wielomiannu
 * @param size2 dÅ‚ugoÅ›Ä‡ danych w arr_2
...
/**
 * Test mnoÅ¼enia dÅ‚ugich wielomianÃ³w.
 * Sprawdza poprawnoÅ›Ä‡ i wydajnoÅ›Ä‡ implementacji
 */
bool MulTest2()
{
    bool good = true;
    ...
    // rozwiÄ…zanie w trybe relase na studentsie wykonuje siÄ™ w 1m44.373s (real) 
...
/**
 * Sprawdza poprawnoÅ›Ä‡ dziaÅ‚ania funkcji PolyIsEq na dÅ‚uÅ¼szych przykÅ‚adach
 * @return
 */
bool IsEqTest()
{
...
/**
 * Sprawdza czy jest uÅ¼ywana implementacja wielomianÃ³w,
 * ktÃ³ra nie zajmuje duÅ¼ej iloÅ›ci pamiÄ™ci dla rzadkich wielomianÃ³w.
 * Test bÄ™dzie uruchamiany z maÅ‚ym limitem pamiÄ™ci.
 */
bool RarePolynomialTest()
{
    bool result = true;
...
// Sekcja z testami tworzonymi rÄ™cznie
...
/**
 * Pomocnicze makro do tworzenia wielomianÃ³w.
...
/**
 * Testuje czy StopieÅ„ wielomianu jest poprawnie liczony rekurencyjnie.
 */
bool DegTest()
{
...
{
    bool res = true;
    // RÃ³Å¼ne przypadki wielomian/wspÃ³Å‚czynnik
    res &= TestAdd(
            C(1),
            C(2),
            C(3));
    ...
    res &= TestAdd(
            ...
            P(C(-1), 1),
            C(2));
    // Dodawanie wspÃ³Å‚czynnika i upraszczanie
    res &= TestAdd(
            C(1),
            P(C(-1), 0, C(1), 1),
            P(C(1), 1));
    ...
    res &= TestAdd(
            ...
            P(P(C(-1), 0, C(1), 1), 0, C(2), 2),
            P(P(C(1), 1), 0, C(2), 2));
    // Dodawanie wielomianÃ³w wielu zmiennych
    res &= TestAdd(
